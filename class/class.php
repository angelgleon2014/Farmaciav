...
GROUP_CONCAT(DISTINCT sucursales.razonsocial SEPARATOR ',') AS sucursales
 FROM sucursales";
    ...
    public function ListarVentas()
    {
        self::SetNames();
        $sql = " SELECT
			ventas.tipodocumento, ventas.codventa, ventas.descuentove,
			ventas.descbonificve, ventas.subtotalve, ventas.totalsinimpuestosve,
			ventas.tarifanove, ventas.tarifasive, ventas.ivave,
			ventas.totalivave, ventas.intereses, ventas.totalpago,
			ventas.totalpago2, ventas.fechavencecredito, ventas.statusventa,
			ventas.fechaventa, clientes.codcliente, clientes.nomcliente,
			cajas.nrocaja, cajas.nombrecaja
			FROM (ventas LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente)
			LEFT JOIN cajas ON ventas.codcaja = cajas.codcaja
			LEFT JOIN detalleventas ON detalleventas.codventa = ventas.codventa
    ...
    public function RegistrarProductos()
    {
        ...
        if ($_POST["precioventacaja"] == "0.00" || $_POST["precioventacaja"] == "0") {
            ...
            if ($num == 0) {
                ...
                $stmt->bindParam(35, $precioventaunidaddesc);
                ...
                $precioventaunidaddesc = $_POST["precioventaunidaddesc"] ? strip_tags($_POST["precioventaunidaddesc"]) : null;
    ...
    public function ListarProductos()
    {
        ...
        if ($_SESSION['acceso'] == "administradorG") {
            ...
            $sql = " SELECT *, fechaexpiracion < NOW() as isExpired, stocktotal < stockminimo as isScarce from productos INNER JOIN presentaciones ON productos.codpresentacion = presentaciones.codpresentacion INNER JOIN medidas ON productos.codmedida = medidas.codmedida LEFT JOIN sucursales ON productos.codsucursal = sucursales.codsucursal LEFT JOIN laboratorios ON productos.codlaboratorio = laboratorios.codlaboratorio WHERE productos.codsucursal = '" . $_SESSION["codsucursal"] . "' AND statusp = 0";
            foreach ($this->dbh->query($sql) as $row) {
                $this->p[] = $row;
            }
            return $this->p;
    ...
    public function BuscarProductosVencidos()
    {
        ...
        if ($var == '0') {
            ...
            $sql = "SELECT * FROM (productos LEFT JOIN sucursales ON productos.codsucursal = sucursales.codsucursal) LEFT JOIN presentaciones ON presentaciones.codpresentacion=productos.codpresentacion LEFT JOIN medidas ON medidas.codmedida=productos.codmedida LEFT JOIN laboratorios ON productos.codlaboratorio = laboratorios.codlaboratorio WHERE productos.stocktotal != 0 AND productos.codsucursal = ? AND fechaexpiracion is not null AND productos.fechaexpiracion <= '" . date("Y-m-d") . "' ORDER BY productos.codproducto";
        } else {

            $sql = "SELECT * FROM (productos LEFT JOIN sucursales ON productos.codsucursal = sucursales.codsucursal) LEFT JOIN presentaciones ON presentaciones.codpresentacion=productos.codpresentacion LEFT JOIN medidas ON medidas.codmedida=productos.codmedida LEFT JOIN laboratorios ON productos.codlaboratorio = laboratorios.codlaboratorio WHERE productos.stocktotal != 0 AND productos.codsucursal = ? AND fechaexpiracion between curdate() and date_add(curdate(), interval '" . $var . "' day) ORDER BY productos.codproducto";
        }
        $stmt = $this->dbh->prepare($sql);
        $stmt->bindValue(1, trim(base64_decode($_GET['codsucursal'])));
        $stmt->execute();
    ...
    ##### funcion para deshabilitar producto de una sucursal ###
    public function DeshabilitarProducto()
    {
        self::SetNames();
        $sql = " UPDATE productos set statusp = 1 WHERE codproducto = ? AND codsucursal = ?";
        ...
        $stmt->execute();
                ...
                echo "<div class='alert alert-info'>";
                echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
                echo "<center><span class='fa fa-check-square-o'></span> EL PRODUCTO FUE DESHABILITADO EXITOSAMENTE </center>";
                echo "</div>";
                exit;
    }
        ...
        if (empty($_SESSION["CarritoC"])) {
            ...
            if ($num == 0) {
                ...
                for ($i = 0; $i < count($compra); $i++) {
                    ...
                    $precio3 = strip_tags($compra[$i]['precioconiva']);
                    ...
                    $precioventablisterdesc = $compra[$i]['precioventablisterdesc'] ? strip_tags($compra[$i]['precioventablisterdesc']) : '0';
                    $precioventacajadesc = $compra[$i]['precioventacajadesc'] ? strip_tags($compra[$i]['precioventacajadesc']) : '0';
                    $precioventaunidaddesc = $compra[$i]['precioventaunidaddesc'] ? strip_tags($compra[$i]['precioventaunidaddesc']) : '0';
                      // var_dump($compra[$i]);
                       //var_dump("PRECIO: " . $precioventaunidaddesc);
                    ...
                    ##################### REGISTRAMOS LOS DATOS DE PRODUCTOS EN KARDEX ####################
                        ...
                        $sql = "select stockunidad, stockblister,stockcajas,blisterunidad, stocktotal, unidades, fechaexpiracion from productos where codproducto = '" . $compra[$i]['txtCodigo'] . "' AND codsucursal = '" . $_POST['codsucursal'] . "'";
                        foreach ($this->dbh->query($sql) as $row) {
                            $this->p[] = $row;
                        }
                        ...
                        $unidadesdb = $row['unidades'];
                        $fechaexpiraciondb = $row['fechaexpiracion'];
			   ...
			   ";
                        ...
                        $stmt->bindParam(20, $precioventablisterdesc);
                        $stmt->bindParam(21, $precioventacajadesc);
                        $stmt->bindParam(22, $precioventaunidaddesc);
                        ...
                        if (!(strtotime($compra[$i]['fechaexpiracion']) > strtotime($fechaexpiraciondb))) {
                            $fechaexpiracion = $fechaexpiraciondb;
                        }
                        $codigobarra = strip_tags($compra[$i]['codigobarra']);
                        if (strip_tags($compra[$i]['codlaboratorio'] == "")) {
                            $codlaboratorio = "1";
    ...
    public function ActualizarDetallesCompras()
    {
        ...
        $stmt->bindParam(23, $codigobarradetalle);
        ...
        if (strip_tags($_POST["fechaexpiracion"] == null)) {
            ...
            $fechaexpiracion = strtotime($_POST['fechaexpiracion']) > strtotime($_POST['fechaexpiracionold'])
                ? date("Y-m-d", strtotime($_POST['fechaexpiracion']))
                : date("Y-m-d", strtotime($_POST['fechaexpiracionold']));
        }
        $codigobarradetalle = strip_tags($_POST['codigobarra']);
    ...
    public function RegistrarArqueoCaja()
    {
        ...
        $num = $stmt->rowCount();
        if ($num == 0) {
            $query = " insert into arqueocaja values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, null); ";
        ...
        if ($num > 0) {
            ?>
							...
							<div class="col-md-6">
								<div class="form-group has-feedback">
									...
									<select class="form-control" onchange="calculaUnidadCosto()" id="tipoVenta" name="tipoventa">
										...
										<option value="UnidadDescuento">Unidad con Descuento</option>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group has-feedback">
									<label class="control-label">P.V.P (Unidad con Descuento): <span class="symbol required"></span> </label>
									<input class="form-control" type="text" name="precioventaunidaddesc" id="precioventaunidaddesc" onKeyPress="EvaluateText('%f', this);" onBlur="this.value = NumberFormat(this.value, '2', '.', '')" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="P.V.P (Blister con Descuento)" readonly="readonly">
									<i class="fa fa-money form-control-feedback"></i>
								</div>
							</div>
    ...
    public function RegistrarVentas()
    {
        ...
        if (empty($_SESSION["CarritoV"])) {
            ...
            $soldProductCodes = array_unique(array_map(function ($venta) {
                return $venta['txtCodigo'];
            }, $v));

            foreach ($soldProductCodes as $productCode) {

                $sql = "select unidades, blisterunidad, stocktotal from productos where codproducto = '" . $productCode . "'";
                foreach ($this->dbh->query($sql) as $row) {
                    $this->p[] = $row;
                }

                $cantventa = 0;
                foreach (array_filter($v, function ($venta) use ($productCode) {
                    return $venta['txtCodigo'] == $productCode;
                })
                    as $venta) {
                    $cantidad = $venta['cantidad'] + $venta['cantidad2'];

                    if ($venta['tipoventa'] == 'Unidad') {
                        $cantventa += $cantidad;
                    } elseif ($venta['tipoventa'] == 'Blister' || $venta['tipoventa'] == 'BlisterDescuento') {
                        $cantventa += $cantidad * $row['blisterunidad'];
                    } elseif ($venta['tipoventa'] == 'Caja' || $venta['tipoventa'] == 'CajaDescuento') {
                        $cantventa += $cantidad * $row['unidades'];
                    }
                }

                $existenciadb = $row['stocktotal'];
                if ($cantventa > $existenciadb) {
                    echo "6";
                    exit;
                }
            }
            ...
            for ($i = 0; $i < count($venta); $i++) {
				...
				$stmt->bindParam(23, strtolower($tipoventa));
                ...
                if ($tipo_venta == 'Unidad' || $tipo_venta == 'UnidadDescuento') {
                    //stockunidad
                    $stockblister = 0;
                    $stockcaja = 0;
                    $blisterunidad = 0;
					...
					precioventaunidad, preciocompraunidad, precioventaunidaddesc,
                    ...
                    foreach ($this->dbh->query($sql2) as $row2) {
                        $this->p[] = $row2;
                        $gananciaConsultada = (float) $row2['ganancia'];
                    }
                    ...
                    foreach ($this->dbh->query($sql1) as $row1) {
                        ...
                        if ($totalUnidad < 0) {
                            ...
                            if ($row1['stockcajas'] > 0) {
                                $cantidadUnitario = $totalUnidad;
                                do {
                                    $cantidadUnitario += (int) $row1['unidades'];
                                } while ($cantidadUnitario < 0);
                            } else {
                        ...
                        $totalGanancia += floatval(
                            (
                                floatval($tipo_venta == 'Unidad' ? $row1['precioventaunidad'] : $row1['precioventaunidaddesc'])
                                - floatval($row1['preciocompraunidad'])
                            ) * (int) $cantidad
                        );
                    }
                    ...
                    $sql = " update productos set "
                        ...
                        . " stocktotal = stocktotal - " . $cantidad . " where "
                        . " codproducto = '" . $venta[$i]['txtCodigo'] . "' and codsucursal = '" . $_POST["codsucursal"] . "';";
                    ...
                    $updateVentas = " update ventas set "
                        . " ganancia = '" . number_format($totalGanancia, 2, '.', '') . "' where "
                    ...
                    foreach ($this->dbh->query($sql) as $row1) {
                        ...
                        $totalGanancia += floatval((
                            floatval($tipo_venta == 'Blister' ? $row1['precioventablister'] : $row1['precioventablisterdesc'])
                            - floatval($row1['preciocomprablister'])
                        ) * (int) $cantidad);
                        ...
                        $totalUnidad = (int) $row1['stockunidad'] - ((int) $row1['blisterunidad'] * (int) $cantidad);
                        ...
                        if ($totalUnidad < 0) {
                            $cantidadUnitario = $totalUnidad;
                            do {
                                $cantidadUnitario += $row1['unidades'];
                            } while ($cantidadUnitario < 0);
                    ...
                    $totalGanancia = $totalGanancia + $gananciaConsultada;

                    $sql = " update productos set "
                        . " stockblister = stockblister - " . ($venta[$i]['cantidad']) . ", "
                        . " stockunidad = " . $cantidadUnitario . ", "
                        . " stockcajas = " . ($totalCajas) . ", "
                        . " stocktotal = stocktotal - " . ($venta[$i]['cantidad']) . " * blisterunidad where "
                        . " codproducto = '" . $venta[$i]['txtCodigo'] . "' and codsucursal = '" . $_POST["codsucursal"] . "';
						";
                    ...
                    foreach ($this->dbh->query($sql) as $row1) {
                        ...
                        $precioVentaCaja = $tipo_venta == 'Caja' ? floatval($row1['precioventacaja']) : floatval($row1['precioventacajadesc']);

                        $totalGanancia += floatval(
                            ($precioVentaCaja
                                - floatval($row1['preciocompra']))
                            * (int) $cantidad
                        );
                    ...
                    $sql = " update productos set "
                        ...
                        . " stocktotal = stocktotal - " . $venta[$i]['cantidad'] . " * unidades  where "
                        . " codproducto = '" . $venta[$i]['txtCodigo'] . "' and codsucursal = '" . $_POST["codsucursal"] . "';
						";
                ...
                $precioventaunidaddesc = strip_tags($venta[$i]['precioventaunidaddesc']);
                $stmt->execute();
                ##################### REGISTRAMOS LOS DATOS DE PRODUCTOS EN KARDEX ####################
            }
            ...
            if ($_POST["tipopagove"] == "CONTADO") {
                ...
                $codcaja = strip_tags($_POST["codcaja"]);
                $stmt->execute();
            } else if ($_POST["tipopagove"] == "CREDITO") {
                require_once('model/Arqueo.php');
                $arqueo = Arqueo::findCurrent();
                $arqueo['ingresos'] += $_POST["montoabono"];
                $arqueo['dineroencredito'] += $_POST["txtTotal"] - $_POST["montoabono"];
                $arqueo->save();
            }
    ...
    public function BusquedaVentas()
    {
        ...
        $sql_columns = "ventas.tipodocumento, ventas.codventa, ventas.descuentove, ventas.descbonificve, ventas.subtotalve, ventas.totalsinimpuestosve, ventas.tarifanove, ventas.tarifasive, ventas.ivave, ventas.totalivave, ventas.intereses, ventas.totalpago, ventas.totalpago2, ventas.fechavencecredito, ventas.statusventa, ventas.fechaventa, clientes.codcliente, clientes.nomcliente, cajas.nrocaja, cajas.nombrecaja ";
        $sql_where = "WHERE 1=1 ";
        $arg_arr = [];

        if (!empty($_GET['codcliente']) || !empty($_GET['busquedacliente'])) {
            $sql_where .= "AND (ventas.codcliente = ? OR clientes.nomcliente LIKE ?) ";
            $arg_arr[] = !empty($_GET['codcliente']) ? $_GET['codcliente'] : -1;
            $arg_arr[] = '%' . (!empty($_GET['busquedacliente']) ? $_GET['busquedacliente'] : '') . '%';
        }
        if (!empty($_GET['codcaja'])) {

            $sql_where .= "AND ventas.codcaja = ? ";
            $arg_arr[] = $_GET['codcaja'];

        }
        if (!empty($_GET['fecha']) && !empty($_GET['fechah'])) {

            $sql_where .= "AND DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') BETWEEN ? AND ? ";
            $arg_arr[] = date("Y-m-d", strtotime($_GET['fecha']));
            $arg_arr[] = date("Y-m-d", strtotime($_GET['fechah']));

        }

        $sql = "
		SELECT $sql_columns
		FROM ventas LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente LEFT JOIN cajas ON ventas.codcaja = cajas.codcaja LEFT JOIN detalleventas ON detalleventas.codventa = ventas.codventa
		$sql_where
		GROUP BY ventas.codventa";
        $stmt = $this->dbh->prepare($sql);
        $stmt->execute($arg_arr);
        $stmt->execute();
        $num = $stmt->rowCount();
        if ($num == 0) {
            ...
            echo "<center><div class='alert alert-danger'>";
            echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
            echo "<span class='fa fa-info-circle'></span> NO EXISTEN VENTAS PARA LA BÚSQUEDA INGRESADA</div></center>";
            exit;
        } else {
    ...
    public function ListarDetallesVentas()
    {
        ...
        $sql = "SELECT detalleventas.coddetalleventa, detalleventas.codventa,
			...
			detalleventas.codpresentacionv,
            detalleventas.tipocantidad,
    ...
    public function BusquedaDetallesVentas()
    {
        ...
        $sql_columns = "detalleventas.coddetalleventa,
        ...
        if (!empty($_GET["codventa"])) {
            $sql_where .= "AND detalleventas.codventa = ? ";
            $arg_arr[] = $_GET["codventa"];
        }
        if (!empty($_GET['codcaja'])) {
            $sql_where .= "AND ventas.codcaja = ? ";
            $arg_arr[] = $_GET['codcaja'];
        ...
        if (!$_SESSION['acceso'] == "administradorG" && !$_SESSION["acceso"] == "administradorS") {
			...
			cajas.nombrecaja
			';
            $sql_where .= "AND ventas.codigo = '" . $_SESSION["codigo"] . "'";
		...
		SELECT $sql_columns
		FROM (detalleventas LEFT JOIN ventas ON detalleventas.codventa = ventas.codventa) LEFT JOIN sucursales ON ventas.codsucursal = sucursales.codsucursal LEFT JOIN presentaciones ON detalleventas.codpresentacionv = presentaciones.codpresentacion LEFT JOIN medidas ON detalleventas.codmedidav = medidas.codmedida
		WHERE $sql_where ORDER BY detalleventas.codventa";
        ...
        if ($num == 0) {
            ...
            echo "<center><div class='alert alert-danger'>";
            echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
            echo "<span class='fa fa-info-circle'></span>NINGUNA VENTA CUMPLE LA BÚSQUEDA INGRESADA</div></center>"; // TODO: mostrar la busqueda
            exit;
        } else {
...
LEFT JOIN cajas ON cajas.codcaja=ventas.codcaja LEFT JOIN clientes ON ventas.codcliente=clientes.codcliente WHERE DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') = '" . date('Y-m-d') . "' AND ventas.codsucursal = '" . $_SESSION["codsucursal"] . "' AND ventas.codigo = '" . $_SESSION["codigo"] . "' GROUP BY detalleventas.codventa";
    ...
    public function RegistrarAbonos()
    {
        ...
        if ($_POST["montoabono"] > $_POST["totaldebe"]) {
            ...
            $sql = "select ingresos, dineroencredito from arqueocaja where codcaja = '" . $_POST["codcaja"] . "' and statusarqueo = '1'";
            foreach ($this->dbh->query($sql) as $row) {
                $this->p[] = $row;
            }
            $ingreso = $row['ingresos'];
            $credito = $row['dineroencredito'];
            $creditoArqueo = $credito - $montoabono;
		...
		";
            $stmt = $this->dbh->prepare($sql);
            $stmt->bindParam(1, $txtTotal);
            $stmt->bindParam(2, $creditoArqueo);
...
(select count(*) from productos where fechaexpiracion is not null AND fechaexpiracion <= '" . date("Y-m-d") . "') as productosvencidos,
...
(select count(*) from productos where fechaexpiracion is not null AND fechaexpiracion <= '" . date("Y-m-d") . "' and codsucursal = '" . $_SESSION["codsucursal"] . "') as productosvencidos,
