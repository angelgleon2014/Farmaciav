...
<?php } ?>
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-tasks"></i> Cuentas por Pagar
...
<li><a href="reportepdf?codsucursal=<?php echo base64_encode($_SESSION["codsucursal"]); ?>&tipo=<?php echo base64_encode("COMPRASXPAGAR") ?>" target="_blank" rel="noopener noreferrer" data-toggle="tooltip" data-placement="left" title="" data-original-title="Listado (Pdf)"><i class="fa fa-file-pdf-o"></i> Listado (Pdf)</a></li>
<li><a href="reporteexcel?codsucursal=<?php echo base64_encode($_SESSION["codsucursal"]); ?>&tipo=<?php echo base64_encode("COMPRASXPAGAR") ?>" data-toggle="tooltip" data-placement="left" title="" data-original-title="Listado (Excel)"><i class="fa fa-file-excel-o"></i> Listado (Excel)</a></li>
            </ul>
</div>
</span><?php } ?></h3>
</div>
                ...
                if(isset($_GET["mesage"])) {
                    switch($_GET["mesage"]) {
                        case 1:
                            ...
                            echo "<center><span class='fa fa-check-square-o'></span> LA FACTURA DE COMPRA FUE PAGADA EXITOSAMENTE </center>";
                            echo "</div>";
                            break;
                        ...
                        case 2:
                            ...
                            echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
                            echo "<center><span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA PAGAR FACTURA DE COMPRAS DE PRODUCTOS, NO ERES EL ADMINISTRADOR DEL SISTEMA</center>";
                            echo "</div>";
                            break;
...
if($reg[$i]['fechavencecredito'] == null) {
    ...
    echo Dias_Transcurridos(date("Y-m-d"), $reg[$i]['fechavencecredito']);
                  ...
                  <td><?php
if($reg[$i]['fechavencecredito'] == null) {
    echo "<span class='label label-success'><i class='fa fa-check'></i> ".$reg[$i]["statuscompra"]."</span>";
} elseif($reg[$i]['fechavencecredito'] >= date("Y-m-d")) {
    echo "<span class='label label-success'><i class='fa fa-check'></i> ".$reg[$i]["statuscompra"]."</span>";
} elseif($reg[$i]['fechavencecredito'] < date("Y-m-d")) {
    echo "<span class='label label-danger'><i class='fa fa-times'></i> VENCIDA</span>";
} ?></td>
                         <td>
<a class="btn btn-success btn-xs" onClick="VerCompra('<?php echo base64_encode($reg[$i]['codcompra']) ?>')" data-href="#" data-toggle="modal" data-target=".bs-example-modal-lg" data-placement="left" data-backdrop="static" data-keyboard="false" data-id="" rel="tooltip" title="Ver Compra"><i class="fa fa-search-plus"></i></a>

<?php if ($_SESSION['acceso'] == "administradorG" || $_SESSION["acceso"] == "administradorS" || $_SESSION["acceso"] == "bodega") { ?>
<a title="Pagar Factura" onClick="pagar('eliminar?<?php echo base64_encode('eliminar'); ?>&codcompra=<?php echo base64_encode($reg[$i]["codcompra"]) ?>&tipo=<?php echo base64_encode("PAGARFACTURA") ?>')" class="btn btn-danger btn-xs"><i class="fa fa-refresh"></i></a>
<?php } ?>

<a href="reportepdf?codcompra=<?php echo base64_encode($reg[$i]['codcompra']); ?>&tipo=<?php echo base64_encode("FACTURACOMPRAS") ?>" target="_black" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Factura de Compra"><i class="fa fa-print"></i></a></td>
											   </tr>
											   <?php  }
} ?>
											   </tbody>
