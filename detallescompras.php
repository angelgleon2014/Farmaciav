...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Detalles de Compras</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Detalles de Compras</li>
</ol>
            ...
            if(isset($_GET["mesage"])) {
                switch($_GET["mesage"]) {
                    ...
                    case 1:
                        ...
                        echo "<span class='fa fa-check-square-o'></span> EL DETALLE DE COMPRA DE PRODUCTO FUE ELIMINADO EXITOSAMENTE </div></center>";
                        break;

                    case 2:
                        echo "<center><div class='alert alert-warning'>";
                        echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
                        echo "<span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR DETALLES DE COMPRAS, NO ERES EL ADMINISTRADOR DEL SISTEMA </div></center>";
                        break;
                }
            }
        ?> 
...
if($reg == "") {
    ...
    for($i = 0;$i < sizeof($reg);$i++) {
        ?>
                    ...
                    <td><?php echo $reg[$i]['codproductoc']; ?></td>
                    <td><?php echo $reg[$i]['productoc']." ".$reg[$i]['nommedida']; ?></td>
                    <td><?php echo $reg[$i]['nompresentacion']; ?></td>
                    <td><?php echo number_format($reg[$i]['preciocomprac'], 2, '.', ','); ?></td>
<td><?php echo $cantidad = ($reg[$i]['cantbonif'] == '0' ? $reg[$i]['cantcompra'] : $reg[$i]['cantcompra']."+".$reg[$i]['cantbonif']); ?></td>
<td><a href="#" class="btn btn-success btn-xs" data-placement="left" title="Ver Detalle Compra" data-original-title="" data-href="#" data-toggle="modal" data-target="#panel-modal" data-backdrop="static" data-keyboard="false" onClick="VerDetalleCompra('<?php echo base64_encode($reg[$i]["coddetallecompra"]); ?>')"><i class="fa fa-search-plus"></i></a>

<?php if ($_SESSION['acceso'] == "administradorG" || $_SESSION["acceso"] == "administradorS" || $_SESSION["acceso"] == "bodega") { ?>

<a href="#" class="btn btn-primary btn-xs" title="Editar" onClick="actualizarc('editdetallecompras?coddetallecompra=<?php echo base64_encode($reg[$i]["coddetallecompra"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<a href="#" class="btn btn-danger btn-xs" onClick="eliminarc('eliminar?<?php echo base64_encode('eliminar'); ?>&coddetallecompra=<?php echo base64_encode($reg[$i]["coddetallecompra"]) ?>&codcompra=<?php echo base64_encode($reg[$i]["codcompra"]) ?>&codsucursal=<?php echo base64_encode($reg[$i]["codsucursal"]) ?>&codproductoc=<?php echo base64_encode($reg[$i]["codproductoc"]) ?>&preciocomprac=<?php echo base64_encode($reg[$i]["preciocomprac"]) ?>&precioventaunidadc=<?php echo base64_encode($reg[$i]["precioventaunidadc"]) ?>&precioventacajac=<?php echo base64_encode($reg[$i]["precioventacajac"]) ?>&unidadesc=<?php echo base64_encode($reg[$i]["unidadesc"]) ?>&cantcompra=<?php echo base64_encode($reg[$i]["cantcompra"]) ?>&cantbonif=<?php echo base64_encode($reg[$i]["cantbonif"]) ?>&unidadesc=<?php echo base64_encode($reg[$i]["unidadesc"]) ?>&ivaproductoc=<?php echo base64_encode($reg[$i]["ivaproductoc"]) ?>&tipo=<?php echo base64_encode("DETALLESCOMPRAS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
                              </tr>
                        <?php  }
} ?>
                              </tbody>
