...
if(isset($_SESSION['acceso'])) {
    if ($_SESSION['acceso'] == "administradorG" || $_SESSION["acceso"] == "administradorS") {
        ...
        if(isset($_POST['btn-submit'])) {
            $reg = $tra->RegistrarIntereses();
            exit;
        } elseif(isset($_POST['btn-update'])) {
            $reg = $tra->ActualizarIntereses();
            exit;
        }
        ?>
<!DOCTYPE html>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Gestión Interés en Tarj.</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li><a href="intereses">Control</a></li>
<li class="active">Interés en Tarj.</li>
</ol>
...
<div class="panel-heading"><h3 class="panel-title"><i class="fa fa-edit"></i> Gestión Interés en Tarj.</h3></div>
<div class="panel-body">
<div class="row">
<div class="col-sm-12 col-xs-12"> 
            <div class="box-body">
			
<?php  if (isset($_GET['codinteres'])) {

    $reg = $tra->InteresesPorId(); ?>
			
      <form class="form" name="updateintereses" id="updateintereses" method="post" data-id="<?php echo $reg[0]["codinteres"] ?>" action="#">
				
		<?php } else { ?>
				
		<form class="form" method="post"  action="#" name="intereses" id="intereses">	
		...
		<?php } ?>
                ...
                <label class="control-label">Tipo de Tarjeta: <span class="symbol required"></span></label>
                              <?php if (isset($reg[0]['tipotarjeta'])) { ?>
<select name="tipotarjeta" id="tipotarjeta" class="form-control" onchange="Verifica();" required="" aria-required="true">
                        <option value="">SELECCIONE</option>
  <option value="DEBITO"<?php if (!(strcmp('DEBITO', $reg[0]['tipotarjeta']))) {
      echo "selected=\"selected\"";
  } ?>>DÉBITO</option>
  <option value="CREDITO"<?php if (!(strcmp('CREDITO', $reg[0]['tipotarjeta']))) {
      echo "selected=\"selected\"";
  } ?>>CRÉDITO</option>
                      </select>
                             <?php } else { ?>  
<select name="tipotarjeta" id="tipotarjeta" class="form-control" onchange="Verifica();" required="" aria-required="true">
            ...
            <label class="control-label">Nombre de Banco: <span class="symbol required"></span></label>
                              <?php if (isset($reg[0]['codbanco'])) { ?>
<select name="codbanco" id="codbanco" class="form-control" onChange="CargaTarjetasxBancos(this.form.codbanco.value,this.form.tipotarjeta.value);" required="" aria-required="true">
                        <option value="">SELECCIONE</option>
      <?php
      $banco = new Login();
                                  ...
                                  for($i = 0;$i < sizeof($banco);$i++) {
                                      ?>
  <option value="<?php echo $banco[$i]['codbanco']; ?>"<?php if (!(strcmp($reg[0]['codbanco'], htmlentities($banco[$i]['codbanco'])))) {
      echo "selected=\"selected\"";
  } ?>><?php echo $banco[$i]['nombanco']; ?></option>        
                      <?php } ?>
                  </select>
                             <?php } else { ?>  
<select name="codbanco" id="codbanco" class="form-control" onChange="CargaTarjetasxBancos(this.form.codbanco.value,this.form.tipotarjeta.value);" disabled="disabled" required="" aria-required="true">
                        <option value="">SELECCIONE</option>
      <?php
      $banco = new Login();
                                 $banco = $banco->ListarBancos();
...
<label class="control-label">Nombre de Tarjeta: <span class="symbol required"></span></label>
 <?php if (isset($reg[0]['codtarjeta'])) { ?>
<select name="codtarjeta" id="codtarjeta" class="form-control" required="" aria-required="true">
                        <option value="">SELECCIONE</option>
      <?php
      $tarjeta = new Login();
     $tarjeta = $tarjeta->ListarTarjetas();
     for($i = 0;$i < sizeof($tarjeta);$i++) {
         ?>
  <option value="<?php echo $tarjeta[$i]['codtarjeta']; ?>"<?php if (!(strcmp($reg[0]['codtarjeta'], htmlentities($tarjeta[$i]['codtarjeta'])))) {
      echo "selected=\"selected\"";
  } ?>><?php echo $tarjeta[$i]['nomtarjeta']; ?></option>        
                      <?php } ?>
                  </select>
                             <?php } else { ?>  
<select name="codtarjeta" id="codtarjeta" class="form-control" disabled="disabled" required="" aria-required="true">
                        <option value="">SELECCIONE</option>
                      </select>
                              <?php } ?>
                              </div> 
                  ...
                  <div class="row">
                            ...
                            <div class="col-md-4"> 
                               <div class="form-group has-feedback"> 
<label class="control-label">N° de Meses: <span class="symbol required"></span></label>
<input type="hidden" name="codinteres" id="codinteres" <?php if (isset($reg[0]['codinteres'])) { ?> value="<?php echo $reg[0]['codinteres']; ?>"<?php } ?>>
<input type="text" class="form-control" name="meses" id="meses" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese N° de Meses"
<?php if (isset($reg[0]['meses'])) { ?> value="<?php echo $reg[0]['meses']; ?>" <?php } else { ?>  disabled="disabled" <?php } ?> required="" aria-required="true">
...
<input type="text" class="form-control" name="tasasi" id="tasasi" onKeyPress="EvaluateText('%f', this);" onBlur="this.value = NumberFormat(this.value, '2', '.', '')"  onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Diferido con Intereses" <?php if (isset($reg[0]['tasasi'])) { ?> value="<?php echo $reg[0]['tasasi']; ?>" <?php } else { ?>  disabled="disabled" <?php } ?> required="" aria-required="true">
                        <i class="fa fa-pencil form-control-feedback"></i>  
                              </div> 
                        </div> 
...
<input type="text" class="form-control" name="tasano" id="tasano" onKeyPress="EvaluateText('%f', this);" onBlur="this.value = NumberFormat(this.value, '2', '.', '')"  onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Diferido sin Intereses" <?php if (isset($reg[0]['tasano'])) { ?> value="<?php echo $reg[0]['tasano']; ?>" <?php } else { ?>  disabled="disabled" <?php } ?> required="" aria-required="true">
                        <i class="fa fa-pencil form-control-feedback"></i>  
                              </div> 
                        </div>	
                    </div><br>
