...
<?php
        ...
        function run_split_sql($uploadfile, $host, $usuario, $passwd)
        {
            ...
            $sql = "";
            // CONEXION
            $conexion = new mysqli('localhost', 'root', '', 'farmaciav', 3306) or die("No se puede conectar con el servidor MySQL: %s\n". $conexion->connect_error);
                ...
                if ("DELIMITER" == substr($line, 0, 9)) {  //empieza por DELIMITER
                    ...
                    if ($respuesta) {
                        ...
                        if (!$conexion->ping()) {
                            ...
                            $respuesta = $conexion->query($sql);
                            if ($respuesta) {
                                echo "<br>$NumLinea REEJECUTADO:  ". str_replace("\n", " ", substr($sql, 0, 130))."...";
                            } else {
...
<button type="reset" class="btn btn-danger waves-effect waves-light"><i class="fa fa-trash-o"></i> Limpiar</button>
                      </div>
                                </form>
								</div>
                          </div>
